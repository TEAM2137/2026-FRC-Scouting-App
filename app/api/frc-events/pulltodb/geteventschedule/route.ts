...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        for (const event of events) {
            ...
            const apiURL = 'https://frc-api.firstinspires.org/v3.0/2026/matches/' + event.code + '?tournamentLevel=Qualification';
            console.log('Fetching Qualification Matches for ' + event.code);
            ...
            for (const match of data.Matches) {
                ...
                match.matchID = event.code + '-' + match.tournamentLevel + '-' + match.matchNumber.toString();

                await connectDB();
                const matchDoc = await Match.findOneAndUpdate({ matchID: match.matchID }, match, { upsert: true, new: true });
            }

            //Get Playoff Matches from Match Results
            const apiURL2 = 'https://frc-api.firstinspires.org/v3.0/2026/matches/' + event.code + '?tournamentLevel=Playoff';
